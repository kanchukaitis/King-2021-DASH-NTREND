function[] = supfigure4
%% Plots the time series for the pseudo-proxy experiments with a CESM target

% Initialize the figure
figure('position', [1 1 1280 643]);

% Load the data
ts = load('time-series-cesm_target.mat');
da = ts.ts.da;
ppr = ts.ts.ppr;
target = ts.ts.target;
...
legend([pd, pp, pt], 'DA', 'PPR', 'CESM Target', 'Location', 'northeast', 'fontsize', 12, 'NumColumns', 3);
legend('boxoff');
